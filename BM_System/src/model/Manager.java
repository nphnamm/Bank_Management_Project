...
public class Manager {
	...
	// khóa tài khoản 
	public int block_account(int acc_num, String cnic)
	{
		DB_Handler db = new DB_Handler();
		...
		{
			return -1; 							//-1 số tài khoản ngân hàng là cnic không trùng khớp
		}
		else if (temp_acc_type == 0 || temp_acc_type == -1)
		{
			return -2;							//-2 số tài khoản hoặc cnic không đúng
		}
		else if (temp_acc_type == 2)
		{
			return -3;							//-3 card đã bị khóa
		}
		else
		{
			db.block_account(acc_num); // thực hiện khóa
		}
		return 0;
	}
	// mở khóa tài khoản
	public int unblock_account(int acc_num, String cnic)
	{
		DB_Handler db = new DB_Handler();
		int temp_client = db.get_client_id(acc_num);
		int temp_acc_type = db.get_account_status(acc_num);
	...
	//hàm khóa thẻ
	public int block_card(int acc_num, String cnic, String card_no)
	{
		DB_Handler db = new DB_Handler();
		...
		{
			return -1;						//-1 Không tìm thấy account với cnic nhập vào 
		}
		else if (temp_card_no != Integer.parseInt(card_no))
		{
			return -2; //thẻ không trùng khớp với thông tin nhập vào
		}
		else if ( db.is_card_active(temp_card_no) == false )
		{
			return -3;  // thẻ bị khóa
		}
		else
		{
			db.block_card(temp_card_no); // thực hiện khóa
		}
		return 0;
	}
	// hàm mở khóa thẻ 
	public int unblock_card(int acc_num, String cnic, String card_no)
	{
		DB_Handler db = new DB_Handler();
		...
		{
			db.unblock_card(temp_card_no); // thực hiện mở khóa
		}
		return 0;
	}
	// hàm đóng tài khoản 
	public int close_account(String account_num, String cnic)
	{
		DB_Handler db = new DB_Handler();	
		//lấy id_client
		...
		{
			db.close_account(Integer.parseInt(account_num),cnic);
			return 0;
		}
		else
			return -1;
